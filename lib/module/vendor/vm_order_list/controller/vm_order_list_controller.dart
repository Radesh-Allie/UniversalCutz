...
class VmOrderListController extends GetxController {
  ...
  doReject(Map item) async {
    ...
    var bookingSnapshot = await FirebaseFirestore.instance
        ...
        .doc(item["id"])
        .get();
    var d = bookingSnapshot.data();
    try {
      if (d["stripe_response"] != null) {
        if (d["stripe_response"]["payment_intent"] != null) {
          var chargeId = d["stripe_response"]["payment_intent"]["charges"]
                  ["data"][0]["id"] ??
              null;

          if (chargeId != null) {
            await PaymentApi.refund(chargeId);
          }
        }
      }
